...
export async function GET(request: NextRequest) {  
  ...
  try {   
    ...
    if (params_kelurahan_kode != null) {
      filteredData = data.filter((item:AreaModel) => item.kelurahan_kode === params_kelurahan_kode);
    }  

    const uniqueData: KelurahanModel[] = [
      ...new Map<string, KelurahanModel>(
        filteredData.map((item: AreaModel) => [
          ...
          { 
            ...
            kelurahan_kode: item.kelurahan_kode.toString(), 
            kelurahan_nama: item.kelurahan_nama 
          }
        ])
      ).values()
    ].sort((a,b) => a.kelurahan_nama.localeCompare(b.kelurahan_nama));
